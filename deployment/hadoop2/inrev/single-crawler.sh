...
# custom vars
export SLAVES=$(cat etc/hadoop/slaves)
export HADOOPUSER=linux
export PATH=$PATH:/home/linux/hadoop/bin
...
# initial injection
cp inrev/1.regex-urlfilter.txt etc/hadoop/regex-urlfilter.txt
cp inrev/1.nutch-default.xml etc/hadoop/nutch-default.xml
for slave in $SLAVES
do
  rsync -tvr etc/hadoop/ $HADOOPUSER@$slave:/home/linux/hadoop/etc/hadoop/
done
...
do
  ...
  # copy regex-urlfilter
  cp inrev/$a.regex-urlfilter.txt etc/hadoop/regex-urlfilter.txt

  # copy nutch-default
  cp inrev/$a.nutch-default.xml etc/hadoop/nutch-default.xml
  for slave in $SLAVES
  do
    rsync -tvr etc/hadoop/ $HADOOPUSER@$slave:/home/linux/hadoop/etc/hadoop/
  done
  ...
  generate_args=($commonOptions -topN $sizeFetchlist $NOF -noNorm -adddays $addDays -crawlId "$CRAWL_ID" -batchId $batchId)
...
## save snapshot before parsing
#  echo "Saving snapshot: "
#  echo "snapshot 'cs_webpage', \"snap-cs-$batchId\"" | /home/linux/hbase/bin/hbase shell
  # parsing the batch
  echo "Parsing : "
